...
class CalculatorLogic
{
  ...
  {
    if(x<=0) return 'Error';
    return _format(math.log(x)/math.log(2));
  }
  ...
  static String evaluateProgrammer(String input)
  {
    try
    {
      ...
      final normalised=input
          ...
          .trim()
          .toUpperCase();
      if(normalised.startsWith('NOT '))
      {
        final operand=normalised.substring(4).trim();
      ...
      for(final op in ops)
      {
        if(normalised.contains(' $op ')) { matchedOp=op; break; }
      }
      if(matchedOp!=null)
      {
        final parts=normalised.split(' $matchedOp ');
  ...
  static String _format(double v,[int precision=10])
  {
    ...
    if(v.abs()>1e300) return 'Error';
    if(v==v.roundToDouble()&&v.abs()<1e15)
    {
      return v.toInt().toString();
    }
    String s=v.toStringAsFixed(precision);
    if(s.contains('.'))
    {
